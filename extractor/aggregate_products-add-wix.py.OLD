...
# variation_retrieval_status enum values:
#   not_applicable — product has no variation system (simple product / single SKU)
#   retrieved      — variation data successfully captured from the source
#   failed         — source advertised variations but they couldn't be fetched
#                    (one fallback row emitted with parent-level price range)
VARIATIONS_NOT_APPLICABLE = "not_applicable"
VARIATIONS_RETRIEVED = "retrieved"
...
def _ci_get(d, *keys, default=None):
    """
    Case-insensitive dict lookup. Returns the first matching value for any
    of `keys`. JSON-LD per spec is case-sensitive, but some generators
    (notably Wix) emit non-standard capitalization like "Offers" / "Availability".
    """
...
# Weak / generic store names returned by various platforms' defaults — when we
# see these we treat them as "not really set" and try a fallback source.
...
def _shopify_product_to_offers(p: dict, site: str, store_name: str, run_id: str, exported_at: str, currency: str) -> list[Offer]:
    ...
    if not variants:
        return [Offer(
            ...
            variation_retrieval_status=VARIATIONS_NOT_APPLICABLE,
    ...
    for v in variants:
        ...
        offers.append(Offer(
            ...
            variation_retrieval_status=VARIATIONS_RETRIEVED,
...
def _woo_product_to_offers(p: dict, session: requests.Session, site: str, store_name: str,
    ...
    return [Offer(
        ...
        currency_minor_unit="" if minor_unit is None else str(minor_unit),
        stock_status=_woo_stock_status(p),
        variation_retrieval_status=VARIATIONS_FAILED,
        purchasable=bool_str(p.get("is_purchasable", None)),
        source_product_url=permalink,
        description=description,
...
def _woo_fetch_variations(session: requests.Session, site: str, store_name: str, product_id: str,
    ...
    """
    Fetch full variation objects for a variable product.

    Primary endpoint (Woo 8.x+ Store API):
        GET /wp-json/wc/store/v1/products?type=variation&parent=<id>
        -> list of full product-shaped objects with prices, permalink, etc.

    Fallback endpoint (older Woo versions):
        GET /wp-json/wc/store/v1/products/<id>/variations
        -> often returns 404 on newer versions; legacy stores may still use it.

    Returns [] if neither endpoint yields variations.
    """
    variations = _woo_get_variation_list(session, site, product_id)
    if not variations:
        return []

    parent_title = parent.get("name", "") or ""
    parent_permalink = parent.get("permalink", "") or ""
    ...
    for v in variations:
        ...
        # variant_title: prefer the API's `variation` string ("Color: Red, Size: L"),
        # fall back to joining the variation attributes ourselves.
        variant_title = (v.get("variation") or "").strip()
        if not variant_title:
            ...
            variant_title = " / ".join(parts)

        variant_raw = dict(raw_base)
        variant_raw["variation_attributes"] = v.get("attributes") or []
        if v.get("variation"):
            variant_raw["variation_label"] = v.get("variation")

        # Prefer Woo's own variation permalink — it already carries the
        # attribute query string. Fall back to synthesizing one ourselves.
        v_permalink = (v.get("permalink") or "").strip()
        url = v_permalink or _woo_variation_url(parent_permalink, parent, v)
...
def _woo_get_variation_list(session: requests.Session, site: str, product_id: str) -> list:
    """Try the modern filter endpoint first, then the legacy sub-endpoint."""
    # Primary: products list filtered by type=variation&parent=<id>
    r = get(session, f"{site}/wp-json/wc/store/v1/products",
    ...
    # Fallback: legacy sub-endpoint (older Woo Store API versions)
...
def fetch_generic_store_name(session: requests.Session, site: str,
                              fallback_product_url: str | None = None) -> str:
    """
    Pull store name from the homepage: prefer og:site_name, fall back to
    application/ld+json Organization/WebSite name, then <title>. If the result
    is a generic platform default (e.g. Wix's "My Site"), additionally probe
    `fallback_product_url`'s JSON-LD for an offer.seller / brand name.
    """
    name = ""
    r = get(session, site + "/")
    if r is not None and r.status_code == 200:
        name = _store_name_from_html(r.text)

    if name.lower() in _GENERIC_STORE_NAMES and fallback_product_url:
        seller = _store_name_from_product_page(session, fallback_product_url)
        if seller:
            return seller
...
def _store_name_from_html(html: str) -> str:
...
def _store_name_from_product_page(session: requests.Session, url: str) -> str:
    """Look for a seller / brand / Organization name in the page's JSON-LD."""
    r = get(session, url)
    if r is None or r.status_code != 200:
        return ""
    for item in extract_jsonld_products(r.text):
        ...
        if isinstance(offers_data, dict):
            candidates.append(offers_data)
        elif isinstance(offers_data, list):
            candidates.extend(x for x in offers_data if isinstance(x, dict))
        for o in candidates:
            seller = _ci_get(o, "seller")
...
def try_generic(session: requests.Session, site: str, run_id: str, exported_at: str) -> list[Offer] | None:
    ...
    if not urls:
        return None
    store_name = fetch_generic_store_name(session, site, fallback_product_url=urls[0])
    offers: list[Offer] = []
    for url in urls:
        r = get(session, url)
        if r is None or r.status_code != 200:
...
def _jsonld_to_offers(item: dict, site: str, store_name: str, url: str, run_id: str, exported_at: str) -> list[Offer]:
    ...
    for o in offer_list:
        if not isinstance(o, dict):
            continue
        price = str(_ci_get(o, "price") or _ci_get(o, "lowPrice") or "")
        ...
        sku = str(_ci_get(o, "sku") or sku_top)

        skip_keys = {"price", "pricecurrency", "availability", "sku"}
        raw = {
            "jsonld_category": category,
            "jsonld_offer": {k: v for k, v in o.items() if isinstance(k, str) and k.lower() not in skip_keys},
        }
