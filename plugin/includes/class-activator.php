...
class Supcomp_Activator {
	...
	public static function activate() {
		...
		// Register all rewrite rules and flush so .htaccess / permalink state
		// picks them up immediately. Re-flushing on every page load would be
		// expensive, so we only do it here.
		Supcomp_Redirect::register_rewrite_rules();
		Supcomp_Canonical_Page::register_rewrite_rules();
		Supcomp_Sitemap::register_rewrite_rules();
		flush_rewrite_rules();
